...
test("mounting nodes panel preserves list state and restores scroll after editor mode", () => {
  ...
  assert.equal(source.includes("renderNodeCardActions"), true);
  ...
  assert.equal(source.includes("buildNodeEditorContext"), true);
  assert.equal(source.includes("buildNodeReturnState"), true);
  ...
  assert.equal(source.includes("Info size={16}"), true);
  assert.equal(source.includes("Box size={16}"), true);
  assert.equal(source.includes("event.stopPropagation()"), true);
  assert.equal(source.includes("onOpenNodeEditor(nodeDetail || node)"), true);
  assert.equal(source.includes("selectedNodeDetail: nodeDetail"), true);
...
test("catalog holes page shows the mounting nodes create mode and editor mode branches", () => {
  ...
  assert.equal(source.includes("handleCreateMountingNode(draft, afterCreate = \"editor\")"), true);
  ...
  assert.equal(source.includes("Зберегти налаштування отворів"), true);
  assert.equal(source.includes("Save hole settings"), true);
  assert.equal(source.includes("selectedNodeDetail: createdNode"), true);
  assert.equal(source.includes("selectedNodeId: String(createdNode.id || \"\")"), true);
  assert.equal(source.includes("mountingNodesViewMode: \"detail\""), true);
  assert.equal(source.includes("nodes: Array.isArray(catalogHolesReturnState?.nodes)"), true);
  assert.equal(source.includes("setCatalogHolesReturnState(nextReturnState);"), true);
  ...
  assert.equal(createPanelSource.includes("holes-workspace-save-actions"), true);
  assert.equal(createPanelSource.includes("mounting-node-create-actions"), true);
  assert.equal(createPanelSource.includes("Зберегти та налаштувати отвори"), true);
  assert.equal(createPanelSource.includes("Зберегти й повернутися до списку"), true);
  assert.equal(createPanelSource.includes("onCreate(draft, afterCreate)"), true);
  assert.equal(createPanelSource.includes("onSubmit={(event) => handleSubmit(event, \"editor\")}"), true);
