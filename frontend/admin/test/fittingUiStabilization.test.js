...
test("fitting ui stabilization keeps refresh, toast, menu, and detail rendering compact", () => {
  ...
  assert.match(appSource, /panelSelector: "\.fitting-action-menu-panel"/);
  assert.match(appSource, /triggerSelector: "\.fitting-action-menu-trigger"/);
  assert.match(appSource, /resetKey: `\$\{activeView\}\|\$\{activeFittingCategory\}\|\$\{fittingViewMode\}`/);
  ...
  const firstFittingsBranchIndex = appSource.indexOf(") : isCatalogFittingsView || isCatalogFastenersView ? (");
  assert.ok(firstFittingsBranchIndex > -1);
  const firstFittingsBranchSnippet = appSource.slice(firstFittingsBranchIndex, firstFittingsBranchIndex + 22000);
  assert.equal((appSource.match(/isCatalogMaterialsView \? \(/g) || []).length, 1);
  assert.equal((appSource.match(/isCatalogFittingsView \|\| isCatalogFastenersView \? \(/g) || []).length, 1);
  assert.match(appSource, /const shouldHideFittingsCatalogOuterToolbar = isCatalogFittingsView \|\| isCatalogFastenersView;/);
  assert.match(appSource, /!isMaterialCleanupView && !isCatalogFittingManufacturersView && !shouldHideFittingsCatalogOuterToolbar && !isCatalogHubView \?/);
  assert.match(appSource, /: activePageLabel && !shouldHideFittingsCatalogOuterToolbar \?/);
  assert.doesNotMatch(appSource, /handleFittingMenuDocumentPointerDown/);
  assert.doesNotMatch(firstFittingsBranchSnippet, /false \? \(/);
  assert.match(firstFittingsBranchSnippet, /fittings-table-shell/);
  assert.match(firstFittingsBranchSnippet, /fittings-toolbar-row/);
  assert.doesNotMatch(firstFittingsBranchSnippet, /material-card-grid|materialsSearch|materialItems/);
